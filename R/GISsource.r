...
#'
#' @param xl x coordinates
#' @param yl y coordinates
#'
#' @return numeric
#' @export
#'
...
#' xl <- c(0,1,2,1)
#' yl <- c(1,2,1,0)
#' plot(xl,yl,type="b") ; polygon(xl,yl)
#' area(xl,yl)
#'
area<-function(xl,yl){
...
#' Return reference number of vector with the nearest value to query value
#'
#' @param list vector data
#' @param q query value
#'
#' @return reference number of vector
#' @export
#'
#' @examples
#' v<-1:10
...
#' v[5]<-5.2;v[6]<-5.2
#' v[nearest(v,q)]
nearest <-function(v,q){
  ans<-which(abs(v-q)==min(abs(v-q)))
  return(ans)
  }
...
#' convert to latitude and longitude of WGS84 fron X, Y of JGD (Japanese Rectangular coordinates)
#'
#' @param lat vector of latitude
#' @param lon vector of longitude
#' @param lat00 original point of latitude for Japanese Rectangular coordinates system
#' @param lon00 original point of longitude for Japanese Rectangular coordinates system
#'
#' @return
#' @export
#'
...
#' rotate of coordinates
#'
#' @param XX original x coordinates
#' @param YY original y coordinates
#' @param x00  x coordinate of a rotation center point
#' @param y00  y coordinate of a rotation center point
#' @param deg  rotation angle
#'
#' @return rotation x coordinates
#' @export rotation y coordinates
#'
#' @examples
#' XX<-c(0,1,2,1,0);YY<-c(1,2,1,0,1);x00<-1;y00<-1;deg<-45
#' xy<- RotateXY(XX,YY,x00,y00,deg)
#' plot(XX,YY,type="b",xlim=c(0,2),ylim=c(0,2))
#' lines(xy,type="b",col="red")
#' points(x00,y00,col="red")
...
#'
#' @param px00 x coordinates interpolated
#' @param py00 y coordinates interpolated
#' @param xaxis x coordinates for reference
#' @param yaxis y coordinates for reference
#'
#' @return
#' @export
#'
#' @examples
#'
#' px00<-1:20 ;  py00<-(1:20)^0.8
#' xaxis <-seq(0,21,0.1) ;yaxis <-seq(0,21,0.1)
#' pxy<-hokan(px00,py00,xaxis,yaxis)
#' plot(px00,py00)
#' points(pxy,col="red",pch=".")
...
#'
#'
#' xaxis <-seq(0,21,.1) ;yaxis <-seq(0,21,.1)
#' px00<-c(1,10,21,10,1) ;  py00<-c(10,20,10,1,10)
#' px00<-c(1,10) ;  py00<-c(10,20)
#' r<-l2r(xaxis,yaxis,px00,py00)
#' image(r)
...
#' Return distance from nearest point in points from a point
#'
#' @param xx0  a point of  x coordinate
#' @param yy0  a point of  y coordinate
#' @param xx vector of x coordinates
#' @param yy vector of y coordinates
#'
#' @return x y coordinates of nearest point  from a point
#' @export
#'
#' @examples
#' xx0 <- 10 ; yy0 <- 10
#' xx <- 1:20 ; yy <- xx+3
#' plot(xx,yy);points(xx0,yy0,col="blue")
#' saitan(xx0,yy0,xx,yy)
#'
...
#' Total length of circumference of polygon XY
#'
#' @param XY XY matrix
#'
#' @return Total length
...
#'
#' @examples
#' XY <- cbind(X=c(1,2,3,2,1),Y=c(1,2,1,0,1))
#' plot(XY,type="b")
#' TotalLength(XY)
